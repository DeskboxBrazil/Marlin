...
// startup. Implementation of an idea by Prof Braino to inform user that any
...
// high temp. !!
// 5 is 100K thermistor - ATC Semitec 104GT-2 (Used in ParCan & J-Head) (4.7k
// pullup)
...
// 12 is 100k 0603 SMD Vishay NTCS0603E3104FXT (4.7k pullup) (calibrated for
// Makibox hot bed)
...
#define PID_MAX                                                                \
  255 // limits current to nozzle while PID is active (see PID_FUNCTIONAL_RANGE
...
#define PID_FUNCTIONAL_RANGE                                                   \
  10 // If the temperature difference between the target temperature and the
     // actual temperature
     // is more then PID_FUNCTIONAL_RANGE then the PID will be shut off and the
...
// hysteresis)
// setting this to anything other than 255 enables a form of PWM to the bed just
// like HEATER_BED_DUTY_CYCLE_DIVIDER did,
// so you shouldn't use it unless you are OK with PWM on your bed.  (see the
...
#define min_software_endstops                                                  \
  false // If true, axis won't move to coordinates less than HOME_POS.
#define max_software_endstops                                                  \
  false // If true, axis won't move to coordinates greater than the defined
        // lengths below.
...
// turned off to avoid jerk
// The value is the delay to turn the servo off after powered on - depends on
// the servo speed; 300ms is good value, but you can try lower it.
// You MUST HAVE the SERVO_ENDSTOPS defined to use here a value higher than zero
// otherwise your code will not compile.

//  #define PROBE_SERVO_DEACTIVATION_DELAY 300

// If you have enabled the Bed Auto Leveling and are using the same Z Probe for
// Z Homing,
...
// - If stepper drivers timeout, it will need X and Y homing again before Z
// homing
// - Position the probe in a defined XY point before Z Homing when homing all
// axis (G28)
...
#define DEFAULT_ACCELERATION                                                   \
  4000 // X, Y, Z and E max acceleration in mm/s^2 for printing moves
...
// #define EXTRUDER_OFFSET_X {0.0, 20.00} // (in mm) for each extruder, offset
// of the hotend on the X axis
// #define EXTRUDER_OFFSET_Y {0.0, 5.00}  // (in mm) for each extruder, offset
// of the hotend on the Y axis
...
//#define SDSLOW // Use slower SD transfer mode (not normally needed - uncomment
//if you're getting volume init error)
//#define SD_CHECK_AND_RETRY // Use CRC checks and retries on the SD
//communication
//#define ENCODER_PULSES_PER_STEP 1 // Increase if you have a high resolution
//encoder
//#define ENCODER_STEPS_PER_MENU_ITEM 5 // Set according to
//ENCODER_PULSES_PER_STEP or your liking
#define ULTIMAKERCONTROLLER // as available from the Ultimaker online store.
//#define ULTIPANEL  //the UltiPanel as on Thingiverse
//#define LCD_FEEDBACK_FREQUENCY_HZ 1000	// this is the tone frequency the
//buzzer plays when on UI feedback. ie Screen Click
//#define LCD_FEEDBACK_FREQUENCY_DURATION_MS 100 // the duration the buzzer
//plays the UI feedback sound. ie Screen Click
...
// header file)
// Note: The PANELOLU2 encoder click input can either be directly connected to a
// pin
...
// frequency
// which is not ass annoying as with the hardware PWM. On the other hand, if
// this frequency
...
// set it manually if you have more servos than extruders and wish to manually
// control some
...
// eliminate calibration or bed height changes.
// Use M206 command to correct for switch height offset to actual nozzle height.
// Store that setting with M500.
//
//#define SERVO_ENDSTOPS {-1, -1, 0} // Servo index for X, Y, Z. Disable with -1
//#define SERVO_ENDSTOP_ANGLES {0,0, 0,0, 70,0} // X,Y,Z Axis Extend and Retract
//angles
